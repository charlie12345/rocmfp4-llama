...
static __device__ __forceinline__ void flash_attn_tile_load_tile_q(
    ...
    constexpr int ne       = 8; // elements dequantized per thread per step
    constexpr int nthreads = warp_size*nwarps;
    constexpr int cpr      = J / ne; // chunks per row
    static_assert(J % ne == 0, "bad J for dequant-on-load");
...
#pragma unroll
    for (int idx0 = 0; idx0 < I*cpr; idx0 += nthreads) {
        ...
        const int j = (idx % cpr)*ne; // element offset within the row
        ...
        if constexpr (std::is_same<T, half2>::value) {
            ...
            if (!oob_check || i < i_sup) {
                dequantize_KV(KV + (size_t)i*stride_KV_bytes, tmp, elem0 + j);
            } else {
#pragma unroll
...
#pragma unroll
            ...
            constexpr dequantize_V_t dequantize_KV = get_dequantize_V<type_KV, float, ne>();

            __align__(16) float tmp[ne];
            if (!oob_check || i < i_sup) {
                dequantize_KV(KV + (size_t)i*stride_KV_bytes, tmp, elem0 + j);
            } else {
#pragma unroll
...
// Prototype: dequant-on-load tile for symmetric quantized KV.
...
extern DECL_FATTN_TILE_CASE_Q(128, 128, GGML_TYPE_Q4_1, GGML_TYPE_Q4_1);
extern DECL_FATTN_TILE_CASE_Q(128, 128, GGML_TYPE_Q5_0, GGML_TYPE_Q5_0);
extern DECL_FATTN_TILE_CASE_Q(128, 128, GGML_TYPE_Q5_1, GGML_TYPE_Q5_1);
extern DECL_FATTN_TILE_CASE_Q(128, 128, GGML_TYPE_Q4_0_ROCMFP4, GGML_TYPE_Q4_0_ROCMFP4);
extern DECL_FATTN_TILE_CASE_Q(128, 128, GGML_TYPE_Q4_0_ROCMFP4_FAST, GGML_TYPE_Q4_0_ROCMFP4_FAST);
